...
The person submitting any changes that impact the current component implementation receives accountability for restablishing consistency: it is their responsability to make sure that all the applications using our component library are swiftly updated. In fact, the modification process will only be considered concluded once the new release of the component library is propagated into all consuming applications.
