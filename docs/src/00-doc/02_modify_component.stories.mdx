...
The person submitting any changes that impact the  receives accountability for restablishing cosinstency: it is their responsability to make sure that all the applications using our component library are swiftly updated. In fact, the modification process will only be considered concluded once the new release of the component library is not propagated into all consuming applications.
