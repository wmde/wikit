...
- Capture the component **states** (e.g. default, hover, active, focus, error, disabled).
...
Figma is the agreed-upon handover environment. In order to collect specs in a centralized way, designers should create a new documentation frame in the Specs and documentation page, and name it after their “Component_Typology” [see example](https://www.figma.com/file/gi3JYwSUQYhKS0mHwK25cQwi/WiKit-%F0%9F%94%A5?node-id=3348%3A259)). 

The following information should be provided in order to carry out an effective handover:

- Component specs fully outlined using tokens (as defined in 1.4):
...
- Storybook indications:
  ...
  - Specify where, within the current Storybook structure, the component should  be classified (new dedicated section or subsection), based on the relationship to its component family.
  - Indicate the required Storybook knobs: these modifiers will allow users to visualize all the component’s visual aspects and behavior
  
- **Assets** should be exported from Figma in an svg format and attached to the component story in Phabricator, so developers can easily download them
